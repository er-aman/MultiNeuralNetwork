...
class MultiNN(object):
    ...
    def add_layer(self, num_nodes, activation_function):
        ...
        #  :return: None
        #  """
        self.weights.append(tf.random.uniform(shape = (self.input_dimension,num_nodes),dtype ='float32'))
        self.biases.append(tf.random.uniform(shape = (num_nodes,),dtype ='float32'))
    ...
    def predict(self, X):
        ...
        for i in range(len(self.weights)):
            net = tf.add(tf.matmul(X,self.weights[i]),self.biases[i])
            X = self.activations[i](net)
        return X
    ...
    def train(self, X_train, y_train, batch_size, num_epochs, alpha=0.8, regularization_coeff=1e-6):
        ...
        for _ in range(num_epochs):
            for i in range(0, X_train.shape[1], batch_size):
                window = i + batch_size
                if window <= X_train.shape[1]:
                    last_index = window
                else:
                    last_index = X_train.shape[1]

                with tf.GradientTape() as tape:
                    batch_y = y_train[i:last_index]
                    batch_x = X_train[i:last_index]
                    prediction = self.predict(batch_x)
                    loss = self.cross_entropy_loss(batch_y,prediction)
                    w, b = tape.gradient(loss,[self.weights,self.biases])
                for j in range(len(self.weights)):
                    self.weights[j].assign_sub(alpha*w[j])
                    self.biases[j].assign_sub(alpha*b[j])
    ...
    def calculate_percent_error(self, X, y):
        ...
        exp = [np.argmax(i, axis=0) for i in pre]
    ...
    def calculate_confusion_matrix(self, X, y):
        ...
        result = np.zeros((self.input_dimension, self.input_dimension))
